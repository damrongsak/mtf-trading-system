...
9.  **🛠️ Infrastructure Guardrails (CRITICAL)**:
    ...
    - **🗄️ Database & Model Guardrails (CRITICAL)**:
        - **Cross-Service Model Sync**: All services sharing the `mtf_db` (e.g., `api-gateway`, `data-pipeline`, `execution`, `strategy-core`) **MUST** have identical SQLAlchemy models for shared entities (`Fund`, `User`, `Trade`, etc.).
        - **Authoritative Migration**: The [data-pipeline](services/data-pipeline) service is the **SOLE Migration Authority**. Before running `alembic revision`, ensure its models in [services/data-pipeline/app/models/](services/data-pipeline/app/models/) are perfectly aligned with [03_data_model.yaml](specs/03_data_model.yaml).
        - **No Partial Model Definitions**: Defining "stubs" or partial models in any service is strictly forbidden. It causes Alembic to generate destructive `DROP` operations for missing fields.
        - **Log-Driven Verification**: After any schema change or model update, you **MUST** check `mtf-postgres` logs (`docker logs mtf-postgres`) to ensure no `column does not exist` or `relation does not exist` errors are being generated by any service.
    - **📊 GEX Data Integrity (v2.5)**:
        - **Reality-Anchoring**: Analysis MUST be anchored to a valid `spot_price` (e.g., $4,738.21). Stale anchors from the $2,800-series are strictly forbidden.
        - **Quarterly Mode**: Default aggregation window is **90 Days (DTE)** to capture significant institutional liquidity walls.
...
*   **Unit & Lot Standardization Rules (CRITICAL)**:
    ...
    - **Official Spec**: Refer to [INSTITUTIONAL_EXECUTION_STANDARD.md](docs/INSTITUTIONAL_EXECUTION_STANDARD.md) for detailed formulas and data flow.
...
- **HIERARCHICAL CONTEXT**: AI agents MUST resolve the `User -> Fund -> Account -> Symbol` hierarchy before execution. Refer to the [INSTITUTIONAL_EXECUTION_STANDARD.md](docs/INSTITUTIONAL_EXECUTION_STANDARD.md) for technical formulas.
...
*   ✅ **Institutional GEX V2.5**: Reality-anchored liquidity surface ($4,750+ spot) with 90-day dynamic aggregation for quarterly structural analysis.
