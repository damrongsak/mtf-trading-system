...
class FeatureEngine:
    ...
    def compute_technicals(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Compute SOTA technical indicators matching paper_experiments.py
        ...
        if arch_model:
            ...
            # We need to handle fitting carefully in production. 
            # For inference, we should ideally use a fixed model, but re-fitting on recent history is acceptable for rolling volatility
            try:
                # Use a sliding window for GARCH to be safe? 
                # Just fit on available buffer
