...
class MergeDecision(BaseModel):
    ...
    should_merge: bool = Field(
        ..., description="True if both nodes represent the same entity"
    )
...
class GraphLinter:
    ...
    async def run_linting_cycle(self) -> Dict[str, Any]:
        ...
        stats = {"orphans_removed": 0, "nodes_merged": 0, "errors": []}
                    ...
                    if self._is_likely_duplicate(node_a, node_b):
                        ...
                        if decision.should_merge:
                            await self._merge_nodes(
                                label, node_a, node_b, decision.canonical_name
                            )
                            merge_count += 1
    ...
    async def _ask_llm_to_merge(
        self, label: str, name_a: str, name_b: str
    ) -> MergeDecision:
        ...
        try:
            ...
            return MergeDecision(
                should_merge=False, canonical_name=name_a, reasoning="LLM failed"
            )

    async def _merge_nodes(
        self, label: str, name_a: str, name_b: str, canonical_name: str
    ):
