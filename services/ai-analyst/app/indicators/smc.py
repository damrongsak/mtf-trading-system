...
def calculate_displacement_velocity(ohlc: pd.DataFrame, period: int = 5) -> pd.Series:
    """
    Calculate the velocity of displacement (V_d) based on candle body size vs high-low range.
    High V_d (> 3 sigma) indicates hyperbolic expansion where reversals are low probability.
    """
    body = (ohlc['close'] - ohlc['open']).abs()
    hl_range = ohlc['high'] - ohlc['low']
    # Velocity = Average Body Size / Average HL Range (normalized momentum)
    # We use z-score of the current body relative to its rolling mean/std to detect expansion
...
def detect_liquidity_sweeps(ohlc: pd.DataFrame) -> List[SMCSweep]:
    ...
    Enhanced with FMEA Guardrails:
    1. Momentum Threshold (V_d > 3 sigma): Reject sweeps during hyperbolic expansion.
    2. Volume Delta Confirmation: Ensure aggressive absorption at the sweep level.
    """
    window = 5
    high = pd.to_numeric(ohlc['high'], errors='coerce')
    low = pd.to_numeric(ohlc['low'], errors='coerce')
    ...
    # 2. Guardrails: Momentum Velocity
    v_d = calculate_displacement_velocity(ohlc)
    v_d_rolling_mean = v_d.rolling(window=20).mean()
    v_d_rolling_std = v_d.rolling(window=20).std()
    # Mask for hyperbolic expansion (V_d > mean + 3*std)
    hyperbolic_mask = v_d > (v_d_rolling_mean + 3 * v_d_rolling_std)
    
    # 3. Guardrails: Volume Delta (Simplified proxy: Volume * Direction)
    # If volume info exists, we want to see high volume on the reversal candle
    has_vol = 'volume' in ohlc.columns
    vol = ohlc['volume'] if has_vol else pd.Series(1, index=ohlc.index)
    avg_vol = vol.rolling(window=20).mean()
    absorption_mask = vol > (avg_vol * 1.2) if has_vol else pd.Series(True, index=ohlc.index)

    # 4. Conditions
    # Bearish Sweep: High > Recent High AND Close < Recent High
    bear_sweep_mask = (high > recent_highs) & (close < recent_highs) & (~hyperbolic_mask) & absorption_mask
    
    # Bullish Sweep: Low < Recent Low AND Close > Recent Low
    bull_sweep_mask = (low < recent_lows) & (close > recent_lows) & (~hyperbolic_mask) & absorption_mask
    ...
    for idx in bear_indices:
        ...
        # Calculate specific metrics for metadata
        v_score = float(v_d.iloc[idx])
        is_hyperbolic = bool(hyperbolic_mask.iloc[idx])
        ...
        sweeps.append({
            ...
            "timestamp": ts_str,
            "level": level,
            "description": "Swept recent high and closed below (FMEA Guarded)",
            "meta": {
                "swept_level": level,
                "wick_size": float(high.iloc[idx] - recent_highs.iloc[idx]),
                "momentum_v_d": v_score,
                "is_hyperbolic_rejection": is_hyperbolic,
                "volume_absorption": float(vol.iloc[idx] / avg_vol.iloc[idx]) if has_vol else 1.0
            }
        })
    ...
    for idx in bull_indices:
        ...
        sweeps.append({
            ...
            "timestamp": ts_str,
            "level": level,
            "description": "Swept recent low and closed above (FMEA Guarded)",
            "meta": {
                "swept_level": level,
