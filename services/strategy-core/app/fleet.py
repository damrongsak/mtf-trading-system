...
class FleetManager:
    ...
    def __init__(self):
        ...
        self.strategy_logs: Dict[str, List[dict]] = {} # id -> list of log records (Ring Buffer)
        self.max_logs_per_strategy = 100
    ...
    def remove_deployment(self, deployment_id: str):
        ...
        if deployment_id in self.strategy_logs:
            del self.strategy_logs[deployment_id]

    def add_strategy_log(self, strategy_id: str, message: str, level: str = "INFO", meta: dict = None):
        """Append a log entry to the strategy's ring buffer."""
        if strategy_id not in self.strategy_logs:
            self.strategy_logs[strategy_id] = []
        ...
        self.strategy_logs[strategy_id].append(log_entry)
        
        # Maintain Ring Buffer size
        if len(self.strategy_logs[strategy_id]) > self.max_logs_per_strategy:
            self.strategy_logs[strategy_id].pop(0)

    def get_strategy_logs(self, strategy_id: str, limit: int = 50) -> List[dict]:
        """Retrieve recent logs for a specific strategy."""
        logs = self.strategy_logs.get(strategy_id, [])
        return logs[-limit:]
        ...
        for strat_id, context in self.active_strategies.items():
            ...
            try:
                ...
                if result is not None:
                    signal_dict = None
                    reason = "No signal generated by logic"
                    logs = None
                    ...
                    if logs:
                        self.add_strategy_log(strat_id, str(logs))
                        # Optional: also publish to gateway if needed for persistence
                        # await gateway_client.publish_logs(strat_id, logs)
