...
class ReconciliationWorker:
    ...
    async def on_execution_event(self, data: dict):
        async with tracing_context(f"worker-event-{data.get('order_id', 'unknown')}"):
            try:
    ...
    async def watchdog_loop(self):
        ...
        while self.is_running:
            async with tracing_context("worker-watchdog"):
                try:
                    db = SessionLocal()
                    # 10 seconds timeout
