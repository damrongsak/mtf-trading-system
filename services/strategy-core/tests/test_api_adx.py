...
def client():
    ...
    with patch("app.workers.indicator_worker.IndicatorWorker") as MockIndicatorWorker, \
         patch("app.main.strategy_engine.start", new_callable=AsyncMock) as mock_se_start, \
         patch("app.main.live_runner.start", new_callable=AsyncMock) as mock_lr_start, \
         patch("app.fleet.FleetManager.load_fleet", new_callable=AsyncMock) as mock_fleet, \
