...
def test_validator_sl_mandatory_missing():
    ...
    with pytest.raises(ValueError, match="Stop Loss is mandatory"):
        OrderValidator.validate("XAUUSD", entry_price=5280.00, sl_price=0, tp_price=5300.00, active_filters=filters)
...
def test_validator_tp_mandatory_missing():
    ...
    with pytest.raises(ValueError, match="Take Profit is mandatory"):
        OrderValidator.validate("XAUUSD", entry_price=5280.00, sl_price=5260.00, tp_price=0, active_filters=filters)
...
def test_validator_sl_distance_failure():
    ...
    with pytest.raises(ValueError, match="SL Distance too small"):
        OrderValidator.validate("XAUUSD", entry_price=5280.00, sl_price=5279.50, tp_price=5300.00, active_filters=filters)

def test_validator_sl_distance_success():
    # SL is 200 pips away ($20)
    filters = [create_filter("SL_DISTANCE", {"min_pip_distance": 200.0})]
    assert OrderValidator.validate("XAUUSD", entry_price=5280.00, sl_price=5260.00, tp_price=5300.00, active_filters=filters) == True
...
def test_validator_rr_ratio_failure():
    ...
    with pytest.raises(ValueError, match="RR Ratio too low"):
        OrderValidator.validate("XAUUSD", entry_price=5280.00, sl_price=5260.00, tp_price=5300.00, active_filters=filters)

def test_validator_rr_ratio_success():
    # Risk is 20, Reward is 40, RR = 2.0, min = 1.5
    filters = [create_filter("RR_RATIO", {"min_ratio": 1.5})]
    assert OrderValidator.validate("XAUUSD", entry_price=5280.00, sl_price=5260.00, tp_price=5320.00, active_filters=filters) == True
...
def test_disabled_filter_ignored():
    ...
    assert OrderValidator.validate("XAUUSD", entry_price=5280.00, sl_price=0, tp_price=5300.00, active_filters=filters) == True
